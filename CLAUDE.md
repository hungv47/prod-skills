...
## Artifacts
Skills write under `skills-resources/` by default; canonical records live in top-level folders:
- `skills-resources/product/flow/<flow-name>.md` — one file per flow (checkout.md, onboarding.md, etc.)
- `skills-resources/product/flow/index.md` — catalog, auto-generated when ≥2 flow files exist
...
- `skills-resources/meta/sketches/prioritize-*.md` (from research-skills) → `system-architecture`: Business initiatives inform what to build
- `skills-resources/product/flow/<flow-name>.md` → `system-architecture`: User flows inform API design and feature decomposition. System-architecture consumes all flow files in the directory.
...
**`--fast` does NOT skip Cold Start.** When no context is resolvable from artifacts or `skills-resources/experience/`, the skill still asks its bundled cold-start questions. `--fast` only bypasses multi-agent orchestration *after* context is resolved — it does not authorize hallucinating against missing platforms / scale targets / deployment context.
